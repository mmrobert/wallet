...
//  Wallet
//
//  Created by boqian cheng on 2017-03-08.
//  Copyright © 2017 Service Ontario. All rights reserved.
//
//  ************************************
//
//   This is the class to create a camera capture session
//     using the AVFoundation framework (camera direct control).
//   For each captured frame output data, using the CIDetector (Core Image)
//   to detect the face and its features (smiling, winking)
//    
//   The viewcontroller (FaceDetectVC) could use this class by notification
//
//  ************************************
...
class Visage: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    fileprivate(set) var imageReturned: CIImage?
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputSampleBuffer sampleBuffer: CMSampleBuffer!, from connection: AVCaptureConnection!) {
        ...
        */
        ...
        if let filter = FaceObscurationFilter(CMSampleBuffer: sampleBuffer), let sourceImage = filter.outputImage {
            
            options = [CIDetectorSmile : true as AnyObject, CIDetectorEyeBlink: true as AnyObject, CIDetectorImageOrientation : 6 as AnyObject]
            
            //
            imageReturned = sourceImage
